...
`sca login`はローカルに一時HTTPサーバー(OSに選ばせたランダムなポート)を立て、
`https://account.lapius7.com/oauth/authorize?redirect_to=http://127.0.0.1:<port>/callback`
をブラウザで開き、account.lapius7.comのSSOハンドオフでログイン後、そのローカルサーバーに
トークンが自動的に返ってくる(`gh`/`aws`等のCLIと同じ方式)。こちらはプロキシを経由せず
...
(`sso-handoff`関数と`ssoRedirect.ts`)、他ドメインへの緩和は一切行っていない。
